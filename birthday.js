...
const FW_COLORS = [
  '#ff4d8d','#ff80b3','#ff006e','#ffb3cc',
  '#ff1493','#ff69b4','#ffd700','#fff0a0',
  '#ff80ff','#cc44ff','#ffffff',
];
...
class FWParticle {
  ...
  draw() {
    for (let i = 0; i < this.trail.length; i++) {
      ...
      fwCtx.beginPath();
      fwCtx.arc(t.x, t.y, this.radius * .5, 0, Math.PI * 2);
      fwCtx.fillStyle = `rgba(255,100,160,${a})`;
      fwCtx.fill();
    }
    fwCtx.beginPath();
...
class Firework {
  ...
  draw() {
    if (this.flash > 0) {
      fwCtx.save();
      fwCtx.globalAlpha = this.flash * .7;
      const g = fwCtx.createRadialGradient(this.x, this.y, 0, this.x, this.y, 40);
      g.addColorStop(0, '#fff');
      g.addColorStop(.4, this.color);
      g.addColorStop(1, 'transparent');
      fwCtx.fillStyle = g;
      fwCtx.beginPath();
      fwCtx.arc(this.x, this.y, 40, 0, Math.PI * 2);
      fwCtx.fill();
      fwCtx.restore();
    }
...
function launchFirework() {
  if (fwCanvas) fireworks.push(new Firework());
}
...
function fwLoop() {
  ...
  if (fwActive || fireworks.length > 0) {
    fwAnimFrame = requestAnimationFrame(fwLoop);
  }
}
...
/* ══════════════════════════════════════════
   MÚSICA - Control de Audio y YouTube
══════════════════════════════════════════ */
...
window.onYouTubeIframeAPIReady = function() {
  ytPlayer = new YT.Player('ytPlayer', {
    ...
    playerVars: {
      autoplay:       0,
      start:          201,   // 3 min 21 seg
      controls:       0,
      disablekb:      1,
      fs:             0,
      modestbranding: 1,
      loop:           1,
      playlist:       'KAANzS240rM',
    },
    events: {
...
function autoplayMusic() {
  ...
  if (musicFab) {
    ...
    musicFab.classList.add('on');
  }

  // Intenta reproducir con YouTube primero
  if (ytReady && ytPlayer && ytPlayer.playVideo) {
    try {
      ytPlayer.seekTo(201, true);
      ytPlayer.playVideo();
      return;
    } catch (e) {
      console.log('Fallo YouTube iframe, usando audio tag.');
    }
  }

  // Respaldar con la etiqueta audio
  if (bdayMusic) {
...
function toggleBdayMusic() {
  if (musicOn) {
    ...
    if (bdayMusic) bdayMusic.pause();
    if (musicFab) {
      musicFab.textContent = '🎵';
      musicFab.classList.remove('on');
    }
    musicOn = false;
  } else {
    autoplayMusic();
...
function typeNameLetters() {
  ...
  function next() {
    if (i >= NAME.length) {
      setTimeout(() => {
        if (introPost) { introPost.textContent = '¡Feliz Cumpleaños! 🎂'; introPost.style.opacity = '1'; }
      }, 400);
      setTimeout(() => {
        if (btnEnter) {
  ...
  setTimeout(() => {
    if (btnEnter && (btnEnter.style.display === 'none' || getComputedStyle(btnEnter).display === 'none')) {
      btnEnter.style.display = 'inline-flex';
      btnEnter.style.opacity = '1';
    }
  }, 4000);
...
function goToMain() {
  ...
  if (sIntro) {
    sIntro.style.transition = 'opacity .9s ease, transform .9s ease';
    sIntro.style.opacity    = '0';
    sIntro.style.transform  = 'scale(1.05)';
  }
...
function openEnvelope() {
  ...
  setTimeout(() => {
    if (wrapper) {
      wrapper.style.transition = 'transform .5s ease, opacity .5s ease';
  ...
  setTimeout(() => {
    ...
    if (envSec) burstHeartsFrom(envSec);
  }, 1100);
}
...
function startBlowDetection() {
  ...
  if (btn) {
    btn.textContent = '🎤 Escuchando... sopla!';
  ...
  navigator.mediaDevices?.getUserMedia({ audio:true, video:false })
    ...
    .catch(() => {
      ...
      if (hint) hint.textContent = '⚠️ Sin micrófono — toca la llama para apagarla';
      const flame = document.getElementById('flameWrapper');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Permite iniciar música en el primer clic a cualquier parte
